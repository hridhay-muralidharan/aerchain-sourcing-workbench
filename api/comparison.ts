import { body, json, readPublic } from './_lib.js';
...
function total(claims: Claim[]) { return Math.round(claims.reduce((sum, claim) => sum + (claim.price === null ? 0 : claim.price * claim.quantity * (claim.currency === 'USD' ? fxRate : 1)), 0) * 100) / 100; }

export default async function handler(req: any, res: any) {
  if (req.method !== 'POST') return json(res, { error: 'POST required' }, 405);
  try {
    await body(req);
    const suppliers = Object.entries(supplierFiles).map(([name, files]) => { const claims = claimsFor(files); const normalizedClaims = claims.map(claim => ({ ...claim, normalizedPrice: claim.price === null ? null : Math.round(claim.price * (claim.currency === 'USD' ? fxRate : 1) * 100) / 100, normalizedCurrency: 'INR' })); return { name, total: total(claims), currency: 'INR', originalCurrency: claims[0]?.currency || null, completeness: claims.length === lineSchedule.length && claims.every(claim => claim.status === 'extracted') ? 'complete' : 'partial', lines: claims.length, claims: normalizedClaims, evidence: files.map(file => ({ label: file.split('/').pop(), path: `/demo-runtime/${file}`, kind: file.split('.').pop()?.toUpperCase() || 'FILE' })) }; });
